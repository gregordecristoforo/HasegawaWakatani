...
diagnostics = @diagnostics [
    progress(; stride=100),
    cfl(; stride=1000, velocity=:burger),
...
# Initialize output
output_file_name = joinpath(@__DIR__, "output", "burgers equation quadratic.h5")
...
function analytical_solution(u0, domain, p, t)
    [abs(y) <= 1 ? 1 - 1 / (4 * t^2) * (1 - sqrt(1 + 4 * t * (t - y)))^2 : 0
     for y in domain.y]
end
...
send_mail("Quadratic burger equation test finished!";
          attachment="figures/burgers steepning quadritic.pdf")
